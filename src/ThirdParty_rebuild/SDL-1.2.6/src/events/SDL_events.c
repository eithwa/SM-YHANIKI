...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: SDL_events.c,v 1.1 2003/09/16 04:36:43 gmaynard Exp $";
#endif
...
int SDL_PrivateOpenGLReset()
{
	int posted;

	posted = 0;
	if ( SDL_ProcessEvents[SDL_OPENGLRESET] == SDL_ENABLE ) {
		SDL_Event event;
		memset(&event, 0, sizeof(event));
		event.type = SDL_OPENGLRESET;
		if ( (SDL_EventOK == NULL) || (*SDL_EventOK)(&event) ) {
			posted = 1;
			SDL_PushEvent(&event);
		}
